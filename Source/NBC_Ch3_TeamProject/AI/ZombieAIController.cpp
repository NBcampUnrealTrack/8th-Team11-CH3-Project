...
void AZombieAIController::BeginPlay()
{
	...
	// BTTask_MoveAndFaceTarget에서 쓸 BB 오브젝트 변수에 값 넣어줌(플레이어를 넘겨준다고 보면 됨)
...
bool AZombieAIController::IsCanAttackSight()
{
	...
	// float DotValue = ZombieFowardDirection | ZombieToPlayerDirection; // 같은 기능

	// 시야안에 들어왔으면 공격중으로 상태를 바꾸고(bIsAttaking) 애니메이션 재생 시간 뒤에 false로 바꿔줌
	if (DotValue >= 0.999f)
	{
		bIsAttaking = true;
